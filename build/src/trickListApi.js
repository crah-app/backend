...
exports.deleteTrickFromTrickList = deleteTrickFromTrickList;
...
async function addTrickToTrickList(req, res, db, userId) {
    ...
    try {
        const results = await new Promise((resolve, reject) => {
            db.query('INSERT INTO Tricks(UserId, Name, Points, Date) VALUES (' + userId + ',\"' + trick.getName() + '\",' + trick.getPoints() + ',' + trick.date + ')', (err, results) => {
                if (err) {
                    reject({ type: errors_1.ErrType.MySqlFailedQuery, message: err.stack ?? undefined });
                }
                else {
...
async function deleteTrickFromTrickList(req, res, db, secret) {
    return (0, userAuth_1.verifyJwt)(req, res, secret, (userId) => {
        removeTrickFromTrickList(req, res, db, userId);
    });
}
async function removeTrickFromTrickList(req, res, db, userId) {
    let trickId = req.body.trickId;
    console.log(userId);
    try {
        const results = await new Promise((resolve, reject) => {
