...
export async function getTrickList(req: Request, res: Response, db: DbConnection): Promise<Err | void> {
...
export async function postTrick(req: Request, res: Response, db: DbConnection, secret: string): Promise<Err | void> {
...
async function postTrickHelper(
	...
	try {
		...
		const query = "START TRANSACTION; INSERT INTO Tricks(UserId, Name, Points, Date) VALUES (?, ?, ?, ?)";

		const trickId = await new Promise<string>((resolve, reject) => {
			db.query(query, [userId, trick.getName(), trick.getPoints(), date], (err: any, results: any) => {
				if (err) {
		...
		for (let i = 0; i < trick.spots.length; i++) {
			...
			const query: string = (i == lastIteration) ? spotQuery + "; COMMIT" : spotQuery;
...
export async function deleteTrick(
	...
	return await verifyJwt(req, res, secret, async (userId: string) => {
		...
		if (!trickId) return await deleteTrickHelper(req, res, db, userId, trickId!);
		return {
			type: ErrType.RequestMissingProperty,
...
export async function deleteTrickHelper(
	...
	const query = "START TRANSACTION; DELETE FROM Tricks WHERE Tricks.Id=? AND Tricks.UserId=?";
